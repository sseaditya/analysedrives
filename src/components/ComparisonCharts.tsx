  ...
  return <div className="space-y-3">
    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
      <div className="min-h-9 flex-1 rounded-lg border border-border/50 bg-muted/30 px-3 py-2 text-sm">
        {displayedPoint ? <div className="flex flex-wrap items-center gap-x-4 gap-y-1 tabular-nums">
          <span className="text-[hsl(var(--segment-drive-1))]"><strong>{matchA.activity.title}</strong> · {displayedPoint.speedA.toFixed(1)} km/h · {displayedPoint.distanceA.toFixed(2)} km · {formatElapsed(displayedPoint.elapsedA)}{displayedPoint.elevationA != null ? ` · ${displayedPoint.elevationA.toFixed(0)} m` : ""}</span>
          <span className="text-[hsl(var(--segment-drive-2))]"><strong>{matchB.activity.title}</strong> · {displayedPoint.speedB.toFixed(1)} km/h · {displayedPoint.distanceB.toFixed(2)} km · {formatElapsed(displayedPoint.elapsedB)}{displayedPoint.elevationB != null ? ` · ${displayedPoint.elevationB.toFixed(0)} m` : ""}</span>
