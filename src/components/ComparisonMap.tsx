...
export default function ComparisonMap({ segment, series, cursor }: { segment: Segment; series: ComparisonSeries; cursor: number }) {
  ...
  return <div className="relative overflow-hidden rounded-xl border"><div ref={elementRef} className="h-[420px] w-full" /><div className="absolute bottom-4 left-1/2 z-[400] flex -translate-x-1/2 gap-3 rounded-full border bg-card/90 px-4 py-2 text-xs shadow-lg backdrop-blur"><span className="flex items-center gap-1"><i className="h-2.5 w-2.5 rounded-full bg-orange-500" />Drive 1</span><span className="flex items-center gap-1"><i className="h-2.5 w-2.5 rounded-full bg-blue-500" />Drive 2</span></div></div>;
}
