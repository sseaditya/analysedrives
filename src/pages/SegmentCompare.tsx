...
export default function SegmentCompare() {
  ...
  useEffect(() => { cursorRef.current = cursorSeconds; }, [cursorSeconds]);
  ...
  useEffect(() => {
    ...
    const animate = (now: number) => {
      ...
      const totalElapsed = Math.max(last.elapsedA, last.elapsedB);
      const remaining = Math.max(1, totalElapsed - cursorStartRef.current);
      const advance = Math.floor(((now - startedRef.current) / 30_000) * remaining);
      const next = cursorStartRef.current + advance;
      if (next >= totalElapsed) { setCursorSeconds(totalElapsed); setPlaying(false); return; }
      setCursorSeconds(next);
  ...
  return <div className="min-h-screen bg-background"><SegmentsHeader backTo={`/segments/${segmentId}`} title="Drive comparison" /><main className="container mx-auto max-w-7xl space-y-6 px-4 py-8">
    ...
    <section className="rounded-2xl border bg-card p-3"><ComparisonMap segment={segmentQuery.data} series={series} cursorSeconds={cursorSeconds} /><div className="mt-4 flex items-center gap-3"><Button size="icon" variant="outline" onClick={() => { setCursorSeconds(0); setPlaying(false); }}><RotateCcw className="h-4 w-4" /></Button><Button size="icon" onClick={() => setPlaying((value) => !value)}>{playing ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}</Button><Slider min={0} max={comparisonDuration} step={1} value={[cursorSeconds]} onValueChange={([value]) => { setCursorSeconds(value); setPlaying(false); }} /><span className="w-20 text-right text-xs tabular-nums text-muted-foreground">{formatDuration(cursorSeconds)}</span></div></section>
    <section className="rounded-2xl border bg-card p-4"><h2 className="mb-2 text-xl font-bold">Speed and elevation</h2><p className="mb-3 text-sm text-muted-foreground">Speed and elevation use the same separate profile layout as Activity. Hover to inspect the comparison.</p><ComparisonTimeline series={series} matchA={matchA} matchB={matchB} onCursor={(value) => { const point = series.points[value]; setCursorSeconds((point.elapsedA + point.elapsedB) / 2); setPlaying(false); }} /></section>
