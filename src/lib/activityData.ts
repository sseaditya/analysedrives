...
export async function loadActivityTrack(activity: ActivitySummary): Promise<LoadedActivity> {
  ...
  if (!processedError && processedBlob) {
    try {
      const parsed = JSON.parse(await processedBlob.text()) as ProcessedTrack;
      ...
      console.warn("Invalid processed activity cache; falling back to GPX", error);
    }
  }
